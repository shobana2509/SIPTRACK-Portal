...
import { Factory, LogIn, ArrowLeft, Eye, EyeOff, Lock, User } from "lucide-react";
import { toast } from "sonner";
import { motion } from "framer-motion";

const LoginIndustryAdmin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      if (user.role !== "industry_admin") {
        toast.error("Access denied. This login is for Industry Admin only.");
        return;
      }
      toast.success(`Welcome, ${user.name}!`);
      navigate("/industry-admin");
    } else {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background p-4">
      <motion.div className="absolute -left-32 -top-32 h-[500px] w-[500px] rounded-full bg-accent/10 blur-[120px]"
        animate={{ x: [0, 40, 0], y: [0, 30, 0], scale: [1, 1.2, 1] }}
        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" as const }} />
      <motion.div className="absolute -bottom-32 -right-32 h-[500px] w-[500px] rounded-full bg-warning/[0.08] blur-[120px]"
      ...
      <motion.div initial={{ opacity: 0, y: 40, scale: 0.92 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.7, ease: "easeOut" as const }} className="relative z-10 w-full max-w-md">
        <Card className="shadow-2xl shadow-accent/5 border-border bg-card/90 backdrop-blur-md overflow-hidden">
          ...
          <CardHeader className="text-center pb-2 pt-8">
            ...
            <motion.div className="mx-auto mb-5 relative" animate={{ y: [0, -8, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" as const }}>
              <div className="flex items-center justify-center rounded-2xl bg-gradient-to-br from-accent to-warning shadow-lg shadow-accent/30 mx-auto" style={{ height: 72, width: 72 }}>
                <Factory className="h-9 w-9 text-accent-foreground" />
              </div>
            </motion.div>
            <CardTitle className="font-display text-2xl">Industry Admin</CardTitle>
            <CardDescription>Industry data management access</CardDescription>
          </CardHeader>
          <CardContent className="pb-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              <motion.div className="space-y-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
                ...
                <div className={`relative rounded-lg transition-all duration-300 ${isFocused === 'username' ? 'ring-2 ring-accent/30' : ''}`}>
              ...
              <motion.div className="space-y-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
                ...
                <div className={`relative rounded-lg transition-all duration-300 ${isFocused === 'password' ? 'ring-2 ring-accent/30' : ''}`}>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                <Button type="submit" className="w-full bg-gradient-to-r from-accent to-warning hover:from-accent/90 hover:to-warning/90 text-accent-foreground shadow-lg shadow-accent/25 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5" size="lg">
                  <LogIn className="mr-2 h-4 w-4" />Sign In as Industry Admin
                </Button>
              </motion.div>
            </form>
          </CardContent>
...
export default LoginIndustryAdmin;
